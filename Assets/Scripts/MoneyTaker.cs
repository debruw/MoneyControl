...
public class MoneyTaker : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            SoundManager.Instance.playSound(SoundManager.GameSounds.LoseMoney);
            if (!isAlone)
            {
                GetComponentInParent<MoneyTakerGroup>().DeactivateTakers();
            }
            if (CollisionManager.Instance.MoneyPiles.Count - needValue >= 0)
            {
            ...
            {
                int forCount = CollisionManager.Instance.MoneyPiles.Count;
                ...
                CollisionManager.Instance.ClosePiles();
                firstmoneyPos.DOMoveZ(firstmoneyPos.position.z + 1, .5f).OnComplete(() =>
                {
                    firstmoneyPos.gameObject.SetActive(false);
                    GameManager.Instance.CheckMoneyAmount();
                });
            }
